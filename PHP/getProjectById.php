<?php
    $host = "localhost"; // Хост базы данных
    $username = "host700505_play"; // Имя пользователя базы данных
    $password = "3469"; // Пароль пользователя базы данных
    $database = "host700505_protocol"; // Имя базы данных
    $connection = new mysqli($host, $username, $password, $database);

    $tk = $connection->real_escape_string($_COOKIE['token']);

    $input_data = file_get_contents('php://input');
    $json_data = json_decode($input_data, true);
    $id = $connection->real_escape_string($json_data['id']);

    // Проверка наличия ошибок при подключении
    if ($connection->connect_error) {
        die('Ошибка подключения к базе данных: ' . $connection->connect_error);
    }

    $result = $connection->multi_query("CALL getInfoProject('$id','$tk')");

    // Вызов хранимой процедуры
    if ($result) {
        $i = 1;
        do {
            // Получение результатов каждого SELECT-запроса
            if ($result = $connection->store_result()) {
                // Преобразование данных в массив
                $data = array();
                while ($row = $result->fetch_assoc()) {
                    $data[] = $row;
                }

                // Добавление данных в ассоциативный массив с именем запроса
                $multiple_res["res" . $i] = $data;
                $result->free();
            }
            $i++;
        } while ($connection->next_result());

        $jsonData = json_encode($multiple_res);
        echo $jsonData;

    } else {
        die("Ошибка при вызове процедуры: " . $connection->connect_error);
        exit();
    }
    $connection->close();
?>